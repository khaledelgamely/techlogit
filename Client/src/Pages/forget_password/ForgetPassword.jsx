...
export default function ForgetPassword() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      await Tech_Logit.post("auth/sendcode", {email:values.email.toLowerCase()})
        .then(() => {
          setValues(values);
          setShowModal(true);
          const [local, domain] = values.email.split("@");
          // Mask all characters in the local part except the first character
          const maskedLocal = local[0] + "*".repeat(local.length - 1);
          // Mask the first character of the domain
          let [maskedDomain, domainParts] = domain.split(".");
          maskedDomain = maskedDomain[0] + "*".repeat(maskedDomain.length - 1);
          // Combine the masked local and domain to form the masked email
          const maskedEmail =
            maskedLocal + "@" + maskedDomain + "." + domainParts;
          setEmail(maskedEmail);
        })
        .catch((err) => {
          setRegisteredError(err.response.data.error);
        });
    } catch (error) {
                  ...
                  {!registeredError ? (
                    ...
                    formik.errors.email && (
                      <div className="text-red-500 flex items-center text-sm">
                        <span className="text-red-700 mx-2 text-sm">
                          <FaExclamationCircle />
                        </span>{" "}
                        {formik.errors.email}
                      </div>
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-center justify-center  p-5  min-w-screen ">
                  <div className="max-w-xl p-8 text-center text-gray-800  bg-white  lg:max-w-3xl rounded-3xl lg:p-12">
                    <h3 className="text-2xl">Rest Account Password</h3>
                    ...
                    <p className="">
                      Success! We've sent an email to <span></span>
                      {email} with password reset instructions.. If the email
                ...
                <div className="flex items-center justify-end p-2 border-t border-solid border-slate-200 rounded-b">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShowModal(false),
                        navigate(
                          `/forget_password/${encodeURIComponent(values.email)}`
                        );
                    }}
                  >
                    Verify Now
